...
function setupColorMode() {
  colorMode = random(['Cat1', 'Cat2', 'Cat3', 'Cat4']);
...
function generateNoiseModifiedColors(count) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (noiseValue > 0.1) {
            let splotchColor = chroma(baseColor)
                .set("rgb.r", (chroma(baseColor).get("rgb.r") + noiseValue) % 255)
                .set("rgb.g", (chroma(baseColor).get("rgb.g") + noiseIntensity * noiseValue) % 255)
                .set("rgb.b", (chroma(baseColor).get("rgb.b") + noiseIntensity * noiseValue) % 255).hex();
...
function selectCurlingFunction() {
    const functions = [curlingSineWaveA, curlingSineWaveB, curlingSineWaveC, curlingSineWaveD, curlingSineWaveE, curlingSineWaveF];
...
function calculateColorSettings(gen, cellHeight, isInsideEllipse, newIsInsideEllipse, isLeftOfDivider, affectCellColor) {
    ...
    switch (colorMode) {
      case 'Cat1':
        ...
        fourColor = chromaCellColor.saturate(cellHeight * 0.1);
        break;
      case 'Cat2':
        zeroColor = chromaCellColor.brighten(cellHeight * 0.08).desaturate(3);
        oneColor = chromaCellColor.desaturate();
        twoColor = chromaCellColor.darken(cellHeight * 0.02).desaturate(3);
        threeColor = chromaCellColor.darken(cellHeight * 0.07).desaturate(3);
        fourColor = chromaCellColor.saturate(cellHeight * 0.1).desaturate(3);
        break;
      case 'Cat3':
        zeroColor = $fx.rand() < 0.5 ? chromaCellColor.brighten(cellHeight * 0.05).saturate(cellHeight * 0.05) : chromaCellColor.brighten(-cellHeight * 0.05).saturate(-cellHeight * 0.05);
        oneColor = adjustColor(chromaCellColor.brighten(cellHeight * 0.1));
        twoColor = $fx.rand() < 0.5 ? chromaCellColor.darken(-cellHeight * 0.05) : chromaCellColor.darken(cellHeight * 0.05);
        threeColor = $fx.rand() < 0.5 ? chromaCellColor.darken(-cellHeight * 0.1) : chromaCellColor.darken(cellHeight * 0.1);
        fourColor = $fx.rand() < 0.5 ? chromaCellColor.saturate(-cellHeight * 0.1) : chromaCellColor.saturate(cellHeight * 0.1);
        break;
      default:
        zeroColor = chromaCellColor;
        oneColor = chromaCellColor;
        twoColor = adjustColor(chromaCellColor.brighten(cellHeight * 0.1));
        threeColor = adjustColor(chromaCellColor);
        fourColor = adjustColor(chromaCellColor.saturate(0.07));
        break;
    }
